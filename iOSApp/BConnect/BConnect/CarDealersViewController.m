...
#import "CarDealersDetailViewController.h"
#import "UILabel+RO.h"
...
@interface CarDealersViewController ()
...
@implementation CarDealersViewController
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.dataLoader = [[ROListDataLoader alloc] initWithDatasource:[[DatasourceManager sharedInstance] screen0DS]
...
- (void)viewDidLoad {
    ...
    [[[ROUtils sharedInstance] analytics] logPage:@"CarDealers"];

    self.title = NSLocalizedString(@"Car Dealers", nil);
...
- (void)configureCell:(ROTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    Screen0DSItem *item = self.items[(NSUInteger)indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Screen0DSItem *item = self.items[(NSUInteger)indexPath.row];
    CarDealersDetailViewController *destinationViewController = [CarDealersDetailViewController new];
