...
@interface HealthClubsViewController ()
...
@implementation HealthClubsViewController
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.dataLoader = [[ROListDataLoader alloc] initWithDatasource:[[DatasourceManager sharedInstance] gymsScreen1DS]
...
- (void)viewDidLoad {
    ...
    [[[ROUtils sharedInstance] analytics] logPage:@"HealthClubs"];

    self.title = NSLocalizedString(@"Health Clubs", nil);
...
- (void)configureCell:(ROTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    GymsScreen1DSItem *item = self.items[(NSUInteger)indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    GymsScreen1DSItem *item = self.items[(NSUInteger)indexPath.row];
    HealthClubsDetailViewController *destinationViewController = [HealthClubsDetailViewController new];
