...
@interface LegalServicesViewController ()
...
@implementation LegalServicesViewController
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.dataLoader = [[ROListDataLoader alloc] initWithDatasource:[[DatasourceManager sharedInstance] lawyersScreen1DS]
...
- (void)viewDidLoad {
    ...
    [[[ROUtils sharedInstance] analytics] logPage:@"LegalServices"];

    self.title = NSLocalizedString(@"Legal Services", nil);
...
- (void)configureCell:(ROTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    LawyersScreen1DSItem *item = self.items[(NSUInteger)indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    LawyersScreen1DSItem *item = self.items[(NSUInteger)indexPath.row];
    LegalServicesDetailViewController *destinationViewController = [LegalServicesDetailViewController new];
